...
public:
    ...
    static shared_ptr<dofCam> create(const String& name = "dofCam");
    ...
    /** \deprecated */
    dofCam(const Matrix4& proj, const CFrame& frame);
    ...
    */
    ...
    /**
     * @brief worldRay: returns world space ray for depth of field calculations. Based around starting point x,y,
     * but uses u,v as camera space starting point.
     * @param x: x coord of main ray.
     * @param y: y coord of main ray.
     * @param u: x coord of start of dof ray
     * @param v: y coord of start of dof ray
     * @param viewport
     * @param filmPlaneDepth
     * @return
     */
    Ray worldRay(float x, float y, float u, float v, const class Rect2D &viewport, float filmPlaneDepth) const;
    ...
    virtual void copyParametersFrom(const shared_ptr<dofCam> &camera);
