...
shared_ptr<dofCam> dofCam::create(const String& name) {
    Any a(Any::TABLE);
    AnyTableReader reader(a);
    return dynamic_pointer_cast<dofCam>(create(name, NULL, reader, ModelTable()));
}

    
shared_ptr<Entity> dofCam::create
(const String&      name,
 Scene*             scene,
 ...
 const Scene::LoadOptions& options) {
     ...
     const shared_ptr<dofCam> c(new dofCam());

     c->Entity::init(name, scene, reader);
     c->dofCam::init(reader);
     reader.verifyDone();
...
Any dofCam::toAny(const bool forceAll) const {
...
float dofCam::circleOfConfusionRadiusPixels(float z, const class Rect2D& viewport) const {
...
dofCam::dofCam(const Matrix4& proj, const CFrame& frame) {
    *this = dofCam();
    m_projection = proj;
    setFrame(frame);
    m_exposureTime =  0.0f;
...
void dofCam::setShaderArgs(UniformTable& args, const Vector2& screenSize, const String& prefix) const {
...
float dofCam::nearPlaneViewportWidth(const Rect2D& viewport) const {
    return m_projection.nearPlaneViewportHeight(viewport);
}


float dofCam::nearPlaneViewportHeight(const Rect2D& viewport) const {
    return m_projection.nearPlaneViewportHeight(viewport);
}


float dofCam::imagePlanePixelsPerMeter(const class Rect2D& viewport) const {
    return m_projection.imagePlanePixelsPerMeter(viewport);
}


// adjusted G3D method to enable depth of field options
Ray dofCam::worldRay(float x, float y, float u, float v, const class Rect2D &viewport,
                     float filmPlaneDepth) const {

    Ray projectedRay = m_projection.ray(x,y,viewport);

    Vector3 origDir = projectedRay.direction();

    float origZ = filmPlaneDepth / (-1.f * origDir.z);
    Vector3 endPt = origDir * origZ;

    Vector3 newOrig = Vector3(u, v, 0.0);

    Vector3 newDir = endPt - newOrig;
    Vector3 normalizedNewDir = normalize(newDir);

    Ray newRay = Ray::fromOriginAndDirection(newOrig, normalizedNewDir);
    newRay = m_frame.toWorldSpace(newRay);
...
Ray dofCam::worldRay(float x, float y, const Rect2D& viewport) const {

    return m_frame.toWorldSpace(m_projection.ray(x, y, viewport));
}


void dofCam::getProjectPixelMatrix(const Rect2D& viewport, Matrix4& P) const {
    m_projection.getProjectPixelMatrix(viewport, P);
}


void dofCam::getProjectUnitMatrix(const Rect2D& viewport, Matrix4& P) const {
    m_projection.getProjectUnitMatrix(viewport, P);
}


Vector3 dofCam::projectUnit(const Vector3& point, const Rect2D& viewport) const {
    return m_projection.projectUnit(m_frame.pointToObjectSpace(point), viewport);
}


Vector3 dofCam::project(const Vector3& point, const Rect2D&  viewport) const {
...
Vector3 dofCam::unprojectUnit(const Vector3& v, const Rect2D& viewport) const {
...
Vector3 dofCam::unproject(const Vector3& v, const Rect2D& viewport) const {
    return m_frame.pointToWorldSpace(m_projection.unproject(v, viewport));
}


float dofCam::worldToScreenSpaceArea(float area, float z, const Rect2D& viewport) const {
    (void)viewport;
    if (z >= 0) {
        return finf();
    }
...
Frustum dofCam::frustum(const Rect2D& viewport) const {
    Frustum f;
    frustum(viewport, f);
    return f;
}
...
void dofCam::frustum(const Rect2D& viewport, Frustum& fr) const {
    m_projection.frustum(viewport, fr);
    fr = m_frame.toWorldSpace(fr);
}

void dofCam::getNearViewportCorners
(const Rect2D& viewport,
 Vector3&      outUR,
 Vector3&      outUL,
 Vector3&      outLL,
...
void dofCam::getFarViewportCorners
   (const Rect2D& viewport,
    Vector3& outUR,
    Vector3& outUL,
    Vector3& outLL,
...
void dofCam::setPosition(const Vector3& t) {
    m_frame.translation = t;
}


void dofCam::lookAt(const Vector3& position, const Vector3& up) {
    m_frame.lookAt(position, up);
}


Vector3 dofCam::convertFromUnitToNormal(const Vector3& in, const Rect2D& viewport) const{
    return (in + Vector3(1.0f, 1.0f, 1.0f)) * 0.5f * Vector3(viewport.width(), viewport.height(), 1.0f) + 
            Vector3(viewport.x0(), viewport.y0(), 0.0f);
}


void dofCam::copyParametersFrom(const shared_ptr<dofCam>& camera) {
...
float dofCam::maxCircleOfConfusionRadiusPixels(const Rect2D& viewport, float viewportFractionMax) {
...
void dofCam::makeGUI(GuiPane* p, GApp* app) {
    ...
    m_app = app;
    entityPane->beginRow(); {
        entityPane->addButton("this = debugCamera", this, &dofCam::onOverwriteCameraFromDebug)->moveBy(-5, 0);
        entityPane->addButton("debugCamera = this", this, &dofCam::onOverwriteDebugFromCamera);
    } entityPane->endRow();
...
#   undef INDENT_SLIDER
    ...
    projectionPane->moveBy(-3, 2);
    {
        ...
        GuiNumberBox<float>* b = projectionPane->addNumberBox("Near Plane Z",
            Pointer<float>(this, &dofCam::nearPlaneZ, &dofCam::setNearPlaneZ),
            "m", GuiTheme::LOG_SLIDER, -20.0f, -0.001f);
        b->setWidth(290);  b->setCaptionWidth(105);

        GuiRadioButton* radioButton;
        b = projectionPane->addNumberBox("Far Plane Z", Pointer<float>(this, &dofCam::farPlaneZ, &dofCam::setFarPlaneZ), "m", GuiTheme::LOG_SLIDER, -1000.0f, -0.10f, 0.0f, GuiTheme::NORMAL_TEXT_BOX_STYLE, true, false);
        b->setWidth(290);  b->setCaptionWidth(105);

        // Field of view
        b = projectionPane->addNumberBox("Field of View", Pointer<float>(this, &dofCam::fieldOfViewAngleDegrees, &dofCam::setFieldOfViewAngleDegrees), GuiText("\xB0", greekFont, 15), GuiTheme::LINEAR_SLIDER, 10.0f, 120.0f, 0.5f);
        b->setWidth(290);  b->setCaptionWidth(105);

        projectionPane->beginRow();
        Pointer<FOVDirection> directionPtr(this, &dofCam::fieldOfViewDirection, &dofCam::setFieldOfViewDirection);
...
void dofCam::onOverwriteCameraFromDebug() {
//    copyParametersFrom(m_app->debugCamera());
}


void dofCam::onOverwriteDebugFromCamera() {
